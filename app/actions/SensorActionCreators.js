...
function getSensorHistory() {
  return getSensor()
    .then(({ sensor }) => {
      return accessBrightWolf(`api/sensors/history?sensor_id=${sensor.sensor_id}&from=${moment().subtract('1', 'days').toISOString()}`)
        .then(({ data }) => {
          AppDispatcher.dispatch({
            actionType: constants.CURRENT_SENSOR_HISTORY_VIEW,
            payload: { history: data.queryresult.matches[0] }
          });
        });
    });
}
